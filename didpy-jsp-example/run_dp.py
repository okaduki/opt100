...
def run_dp_jsp():
    ...
    solver = DpJspSolver2(problem=problem)
    ...
    df = pd.DataFrame(
        [
            dict(Task=f"Job {i}", Start=task[0], Finish=task[1], SubTask=j)
            for i in range(len(sched))
            for j, task in enumerate(sched[i])
        ]
    )

    # fig = ff.create_gantt(df, index_col="SubTask", group_tasks=True, show_colorbar=True)
    # fig.write_image("jsp.png")
    # fig.show()
